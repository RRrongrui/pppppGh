...
public class PondItemAdapter extends BaseAdapter {
    ...
    private void initializeViews(Pond.DataBean object, ViewHolder holder) {
        ...
        imageLoad.HttpImage("http://m.yundiaoke.cn"+object.getContent(),holder.pondImg);
        if (object.getCharge()!=null){
            if (object.getCharge().equals("0")){
                holder.pondMony.setText(object.getPrice()+"/斤");
            }else{
                holder.pondMony.setText(object.getPrice()+"/天");
            }
        }else {holder.pondMony.setText("无");}
